...
class PontoRiscoList extends TPage
{
    private $form; // form
    ...
    private static $formName = 'formList_PontoRisco';
    ...
    public function __construct()
    {
        ...
        $this->form->setFormTitle('Listagem de ponto riscos');
        ...
        $row2 = $this->form->addFields([new TLabel('Ponto id:', null, '14px', null)],[$ponto_id],[new TLabel('Risco id:', null, '14px', null)],[$risco_id]);
        ...
        $btn_onshow = $this->form->addAction('Cadastrar', new TAction(['PontoRiscoForm', 'onShow']), 'fa:plus #69aa46');
        ...
        $column_risco_risco = new TDataGridColumn('risco->risco', 'Risco id', 'left');
        ...
        $this->datagrid->addColumn($column_risco_risco);

        $action_onEdit = new TDataGridAction(array('PontoRiscoForm', 'onEdit'));
        ...
        $action_onDelete = new TDataGridAction(array('PontoRiscoList', 'onDelete'));
    ...
    { 
        if(isset($param['delete']) && $param['delete'] == 1)
        {
            try
            {
                ...
                // instantiates object
                $object = new PontoRisco($key, FALSE); 
    ...
    public function onSearch()
    {
        ...
        if (isset($data->risco_id) AND ( (is_scalar($data->risco_id) AND $data->risco_id !== '') OR (is_array($data->risco_id) AND (!empty($data->risco_id)) )) )
        {

            $filters[] = new TFilter('risco_id', '=', $data->risco_id);// create the filter 
        }
    ...
    public function onReload($param = NULL)
    {
        try
        {
            ...
            // creates a repository for PontoRisco
