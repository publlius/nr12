...
class DatabaseInstall extends TPage
{
    ...
    public static function install($params)
    {
        try 
        {
            ...
            foreach ($params['name'] as $key => $name) 
            {
                ...
                if(file_exists("app/database/{$databaseName}-{$ini['type']}.sql"))
                {   
                    $fks = self::createTables("app/database/{$databaseName}-{$ini['type']}.sql", $name, $ini['type'], $databaseName);
                }
                else
                {
                    $fks = self::createTables("app/database/{$databaseName}.sql", $name, $ini['type'], $databaseName);
                }
                ...
                if($params['database_name'][$key] == 'permission')
                {
                    ...
                    self::updateModels($params['database_type'][$key]);
                ...
                if( $fks)
                {
                    self::createFKs( $fks, $name, $ini['type'], TTransaction::get(), $databaseName );
                }
    ...
    public static function insertDefaultData($name, $databaseName, $databaseType)
    {
        ...
        {            
            ...
            if(!isset($inserted["{$databaseName}_{$name}_inserts"]) || (isset($inserted["{$databaseName}_{$name}_inserts"]) && $inserted["{$databaseName}_{$name}_inserts"] == 0 ))
            {
                ...
                $inserted["{$databaseName}_{$name}_inserts"] = 1;
            }
