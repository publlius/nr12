...
$query_rs_origem = " SELECT 
						...
						estado.uf
							 FROM (SELECT @eqp := 0) e, apreciacao
						...
						LEFT JOIN estado ON cidade.estado_id = estado.id
						    WHERE empresa.id = '".$_REQUEST[empresa_id]."' 
						";
...
$query_rs_eqp = " SELECT 
						...
						LEFT JOIN estado ON cidade.estado_id = estado.id
						    WHERE empresa.id = '".$_REQUEST[empresa_id]."' 
						";
$rs_eqp = mysqli_query($n12, $query_rs_eqp);
$row_rs_eqp = mysqli_fetch_assoc($rs_eqp);
$totalRows_rs_eqp = mysqli_num_rows($rs_eqp);
//$n12->close();
// ********** FIM EQUIPAMENTOS ******************** \\
...
<?php  //do { 
// ********** INÍCIO PERIGOS ***************** \\
$query_rs_spaf = " SELECT 
                          @pto := @pto + 1 pto,
							ponto.id, ponto.apreciacao_id,
                            ponto_sistema_seguranca.*,
                            tipo_sistema_seguranca.*,
                            sistema_seguranca.*,
                            apreciacao.*,
                            equipamento.*
							FROM 
                                ...
                                LEFT JOIN tipo_sistema_seguranca ON ponto_sistema_seguranca.ponto_id = tipo_sistema_seguranca.id
                                LEFT JOIN sistema_seguranca ON tipo_sistema_seguranca.id = sistema_seguranca.tipo_sistema_seguranca_id
                                LEFT join apreciacao ON ponto.apreciacao_id = apreciacao.id
                                LEFT JOIN equipamento ON apreciacao.equipamento_id = equipamento.id
                           WHERE equipamento.empresa_unidade_id = '".$_REQUEST['empresa_id']."' 
                             AND equipamento.id = '".$row_rs_origem['equipamento_id']."' 
                             AND ponto.localizacao_ponto = 'Frontal' 
                             AND sistema_seguranca.tipo_sistema_seguranca_id != '13'
                           ";
$rs_spaf = mysqli_query($n12, $query_rs_spaf);
$row_rs_spaf = mysqli_fetch_assoc($rs_spaf);
$totalRows_rs_spaf = mysqli_num_rows($rs_spaf);

$query_rs_spale = " SELECT 
                          @pto := @pto + 1 pto,
							ponto.id, ponto.apreciacao_id,
                            ponto_sistema_seguranca.*,
                            tipo_sistema_seguranca.*,
                           ...
                           WHERE equipamento.empresa_unidade_id = '".$_REQUEST['empresa_id']."' 
                             AND equipamento.id = '".$row_rs_origem['equipamento_id']."' 
                             AND ponto.localizacao_ponto = 'Lateral esquerda' 
                             AND sistema_seguranca.tipo_sistema_seguranca_id != '13'
                           ";
$rs_spale = mysqli_query($n12, $query_rs_spale);
$row_rs_spale = mysqli_fetch_assoc($rs_spale);
$totalRows_rs_spale = mysqli_num_rows($rs_spale);

$query_rs_spald = " SELECT 
                          @pto := @pto + 1 pto,
							ponto.id, ponto.apreciacao_id,
                            ponto_sistema_seguranca.*,
                            tipo_sistema_seguranca.*,
                           ...
                           WHERE equipamento.empresa_unidade_id = '".$_REQUEST['empresa_id']."' 
                             AND equipamento.id = '".$row_rs_origem['equipamento_id']."' 
                             AND ponto.localizacao_ponto = 'Lateral direita' 
                             AND sistema_seguranca.tipo_sistema_seguranca_id != '13'
                           ";
$rs_spald = mysqli_query($n12, $query_rs_spald);
$row_rs_spald = mysqli_fetch_assoc($rs_spald);
$totalRows_rs_spald = mysqli_num_rows($rs_spald);

$query_rs_spap = " SELECT 
                          @pto := @pto + 1 pto,
							ponto.id, ponto.apreciacao_id,
                            ponto_sistema_seguranca.*,
                            tipo_sistema_seguranca.*,
                           ...
                           WHERE equipamento.empresa_unidade_id = '".$_REQUEST['empresa_id']."' 
                             AND equipamento.id = '".$row_rs_origem['equipamento_id']."' 
                             AND ponto.localizacao_ponto = 'Posterior' 
                             AND sistema_seguranca.tipo_sistema_seguranca_id != '13'
                           ";
$rs_spap = mysqli_query($n12, $query_rs_spap);
$row_rs_spap = mysqli_fetch_assoc($rs_spap);
$totalRows_rs_spap = mysqli_num_rows($rs_spap);
// ********** FIM Perigos ******************** \\
...
<table colspan="12" border="0" align="center" width="750px">
	...
	<tr><td colspan="12" align="left" class="laudo_title"><b>3. Identificação da Máquina</b></td></tr>
	<tr><td colspan="12" align="left" class="laudo_title"><b>Equipamento <?php echo utf8_encode($row_rs_origem['eqp']);?>: <?php echo utf8_encode($row_rs_origem['equipamento_nome']);?></b></td></tr>
	...
	<tr><td colspan="12" align="left" class="laudo_title"><b>Sistemas de Proteção Aplicados</b></td></tr>

	<tr class="border_bottom">
		<td colspan="6" align="left">Face Frontal<br>
			<?php  do { ?>
				<?php echo utf8_encode($row_rs_spaf['sistema_seguranca']);?><br>
			<?php  } while ($row_rs_spaf = mysqli_fetch_assoc($rs_spaf)); ?>
		</td>
		<td colspan="6" align="left">Face Lateral Esquerda<br><br>
			<?php  do { ?>
				<?php echo utf8_encode($row_rs_spale['sistema_seguranca']);?><br>
			<?php  } while ($row_rs_spale = mysqli_fetch_assoc($rs_spale)); ?>
		</td>
		</td>
	</tr>
	<tr class="border_bottom">
		<td colspan="6" align="left">Face Lateral Direita<br>
			<?php  do { ?>
				<?php echo utf8_encode($row_rs_spald['sistema_seguranca']);?><br>
			<?php  } while ($row_rs_spald = mysqli_fetch_assoc($rs_spald)); ?>
		</td><br>
		</td>
		<td colspan="6" align="left">Face Posterior<br>
			<?php  do { ?>
				<?php echo utf8_encode($row_rs_spap['sistema_seguranca']);?><br>
			<?php  } while ($row_rs_spap = mysqli_fetch_assoc($rs_spap)); ?>
		</td><br></td>
	</tr>
